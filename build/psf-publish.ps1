...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'PSmac' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\PSmac" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the PSmac module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\PSmac"
