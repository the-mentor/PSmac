...
if (-not (Test-Path -Path "$WorkingDirectory\publish\PSmac")) {
...
$moduleVersion = (Import-PowerShellDataFile -Path "$WorkingDirectory\publish\PSmac\PSmac.psd1").ModuleVersion

# Step 1: Zip Module Content
Write-Host "Wrapping up built module into a zip archive"
Compress-Archive -Path "$WorkingDirectory\publish\PSmac\*" -DestinationPath "$WorkingDirectory\publish\PSmac.zip" -Force

# Step 2: Create Release
Write-Host "Registering new release for version $($moduleVersion) with Github"
$response = Invoke-RestMethod -Method POST -Uri 'https://api.github.com/repos/the-mentor/PSmac/releases' -Headers @{
	...
	'X-GitHub-Api-Version' = '2022-11-28'
		...
		body        = "Releasing v$moduleVersion of the PSmac module."
		make_latest = 'true'
...
Invoke-RestMethod -Method POST -Uri "$($response.assets_url -replace 'api\.github\.com', 'uploads.github.com')?name=PSmac.zip" -Headers @{
	Authorization          = "Bearer $env:GH_TOKEN"
	Accept                 = 'application/vnd.github+json'
	'X-GitHub-Api-Version' = '2022-11-28'
	'Content-Type'         = 'application/octet-stream'
} -InFile "$WorkingDirectory\publish\PSmac.zip"
